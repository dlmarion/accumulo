...
public class TraceServer implements Watcher, AutoCloseable {
    ...
    while (true) {
      try {
        ...
        if (loginMap.isEmpty() && isDefaultTokenType) {
          ...
          AuthenticationToken token = Thread.currentThread().getContextClassLoader()
  ...
  private static void loginTracer(AccumuloConfiguration acuConf) {
    try {
      Class<? extends AuthenticationToken> traceTokenType = Thread.currentThread()
          .getContextClassLoader().loadClass(acuConf.get(Property.TRACE_TOKEN_TYPE))
